...
class MemoRouter:
    ...
    def __init__(self, storage: MemoStorage) -> None:
    ...
    def _setup_routes(self) -> None:
        ...
        async def update_memo(memo_id: str, request: UpdateMemoRequest) -> Memo:
        ...
        async def set_memo_attachments(memo_id: str, body: SetMemoAttachmentsBody) -> Dict[str, str]:
        ...
        async def set_memo_relations(memo_id: str, body: SetMemoRelationsBody) -> Dict[str, str]:
            """Set memo relations."""
            memo = self.storage.get(memo_id)
            if not memo:
                raise HTTPException(status_code=404, detail="Memo not found")
        ...
        async def upsert_memo_reaction(memo_id: str, body: UpsertMemoReactionBody) -> Dict[str, str]:
            ...
            existing = next((r for r in memo.reactions if r.creator == body.reaction.creator), None)
            if existing:
        ...
        async def rename_memo_tag(body: RenameMemoTagBody) -> Dict[str, str]:
            """Rename memo tags across all memos."""
            for memo in self.storage.memos.values():
                if memo.tags:
                    memo.tags = [body.new_tag if tag == body.old_tag else tag for tag in memo.tags]
            return {"message": "Tags renamed successfully"}

        @self.router.delete("/memos/-/tags/{tag}")
        async def delete_memo_tag(tag: str, delete_related_memos: bool = Query(False)) -> Dict[str, str]:
