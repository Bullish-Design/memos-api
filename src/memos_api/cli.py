...
@memo_app.command("create", no_args_is_help=True)
def memo_create(
    ...
    async def _create():
        ...
        async with APIClient(url, token) as client:
            response = await client.client.post(
                "/api/v1/memos", 
                json={"content": content, "visibility": visibility, "state": "NORMAL"}
            )
...
@memo_app.command("get", no_args_is_help=True)
def memo_get(
...
@user_app.command("create", no_args_is_help=True)
def user_create(
